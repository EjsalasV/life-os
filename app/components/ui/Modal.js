...
export default function Modal({ isOpen, onClose, title, children, busy = false, adventure = false }) {
  const titleId = useId();
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center sm:p-4">
      <section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-busy={busy}
        className={`w-full max-w-md max-h-[90dvh] overflow-y-auto overscroll-contain rounded-t-3xl bg-white p-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-2xl dark:bg-gray-900 sm:rounded-3xl ${adventure ? "modal-adventure-business" : ""}`}>
