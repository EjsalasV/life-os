 ...
 * Animaciones por CSS (compositor): funcionan aunque rAF esté suspendido,
 * y el footer va en flujo para que nunca se solape con el texto.
 */
export default function Onboarding({ onFinish, userName }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-white dark:bg-gray-950 flex flex-col items-center justify-center gap-12 p-8 overflow-y-auto">
      ...
      {/* SLIDE ACTUAL */}
      <div key={step} className="w-full max-w-sm text-center space-y-10 relative z-10 animate-fade-in-scale">
        <div className={`w-28 h-28 ${slides[step].color} rounded-[40px] flex items-center justify-center mx-auto shadow-sm border border-white/50 animate-pet-pulse`}>
          {slides[step].icon}
        </div>
      ...
      {/* FOOTER DE NAVEGACIÓN (en flujo: nunca se solapa con el contenido) */}
      <div className="w-full max-w-sm px-8 space-y-8 relative z-10">
        ...
        <button
          onClick={handleNext}
          className="w-full bg-black dark:bg-white dark:text-black text-white py-5 rounded-[24px] font-black uppercase text-[12px] tracking-[0.2em] shadow-2xl flex items-center justify-center gap-3 transition-all active:scale-95"
        >
