...
export default function AuthView({ onLogin, onRegister, loading, error }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--life-surface)] flex flex-col items-center justify-center p-6 pb-12">
      ...
      <div className="text-center mb-10">
         ...
         <h1 className="text-3xl font-black tracking-tighter text-[var(--life-text)] uppercase">Life OS</h1>
         <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--life-text-muted)] mt-1 italic">Version 1</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!isLogin && (
            <div className="relative">
              <User className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--life-text-muted)]" size={18}/>
              <input
                type="text"
                ...
                autoComplete="name"
                required
                className="w-full bg-[var(--life-surface)] p-4 pl-12 rounded-2xl outline-none font-bold text-sm focus:ring-2 focus:ring-black/5 transition-all" 
                value={name} 
          ...
          <div className="relative">
            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--life-text-muted)]" size={18}/>
            <input
              type="email"
              placeholder="Email"
              ...
              autoComplete="email"
              required
              className="w-full bg-[var(--life-surface)] p-4 pl-12 rounded-2xl outline-none font-bold text-sm" 
              value={email} 
              onChange={(e)=>setEmail(e.target.value)} 
            />
          </div>
          <div className="relative">
            <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--life-text-muted)]" size={18}/>
            <input
              type="password"
      ...
      <div className="w-full max-w-md mt-16 space-y-6">
         <p className="text-[9px] font-black text-[var(--life-text-muted)] uppercase tracking-[0.4em] text-center mb-2 italic">Explora el ecosistema</p>
            ...
            {features.map((f, i) => (
              <div key={i} className="bg-white p-5 rounded-[32px] shadow-sm border border-gray-100 flex flex-col items-center text-center gap-3 transition-transform hover:scale-105">
                 ...
                 <div className="space-y-1">
                    <p className="text-[11px] font-black text-[var(--life-text)] leading-tight uppercase">{f.l}</p>
                    <p className="text-[7px] font-bold text-[var(--life-text-muted)] uppercase tracking-tighter leading-none">{f.desc}</p>
                 </div>
              </div>
            ))}
         </div>
